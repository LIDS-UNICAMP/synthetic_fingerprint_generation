...
def generate_synthetic_latent(img, args):
    ...
    if args.occlusion:
        ...
        cv2.imwrite('occluded.png', out)  # Debugging line, can be removed later
    if args.thickness:
        out = vary_ridge_thickness(out)
        cv2.imwrite('thickness_variation.png', out)  # Debugging line, can be removed later
    if args.illumination:
        out = gamma(out, value=gamma_value)
        cv2.imwrite('gamma_correction.png', out)  # Debugging line, can be removed later
    if args.degradation:
        out = gaussian_blur(out, kernel_size=blur_size)
        cv2.imwrite('gaussian_blur.png', out)
    if args.illumination:
        out = downsampling(out, block_size=downsample)
        cv2.imwrite('downsampling.png', out)  # Debugging line, can be removed later
    if args.degradation:
        out = gaussian_noise_speckle(out, noise_level=noise_level)
        cv2.imwrite('gaussian_noise.png', out)  # Debugging line, can be removed later
    if args.background:
        out = add_background_image(out)
        cv2.imwrite('background_added.png', out)
